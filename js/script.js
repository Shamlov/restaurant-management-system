...
function showMenuCategories(categories) {     // категории заполняются автоматически из массива
    categories.forEach((el, index) => {
        menuCategories.insertAdjacentHTML('afterEnd', `<div class="menu-category"><h4 class="menuCategory d-inline-block">${el}</h4></div>`)
    });
            ...
            menuCategory[i].insertAdjacentHTML('afterEnd', 
                `<div class="order-card d-flex card-design my-1 p-1 rounded">
                    ...
                    <div class="card-button-block d-flex flex-column px-3 justify-content-center">
                        <button class="edit btn btn-success mb-1 p-1 fs-5" type="button" data-bs-toggle="modal" data-bs-target="#exampleModal">Редактировать</button>
                    </div>
                </div>`
            )
        }
